...
class BackTranslation:
    ...
    def translate(self, sample, back: bool = False):
        ...
        Returns:
            str: Translated text.
        """
        tokens = self.src_tokenizer(sample, return_tensors="pt", padding=True) if back is not True else \
            self.tgt_tokenizer(sample, return_tensors="pt", padding=True)
        tokens = tokens.to(self.device)

        translated = self.src_translator.generate(**tokens, max_new_tokens=MAX_MODEL_LENGTH) if back is not True else \
